...
/**
 * TRANG 2: BẢNG ĐIỀU KHIỂN GIÁM SÁT (MONITORING DASHBOARD)
 * ------------------------------------------------------
 * Chuyển đổi từ hiển thị tĩnh sang "Trung tâm điều hành của Agent":
 * 1. Strategic Goal: Hiển thị mục tiêu cốt lõi < 0.5 g/L.
 * 2. Metrics: Bổ sung trạng thái phân tích của AI cho từng chỉ số.
 * 3. AI Sentinel: Tích hợp trực tiếp luồng suy luận Chain-of-Thought.
 */
...
export const Dashboard = () => {
  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-1000">
      {/* --- 1. MỤC TIÊU CHIẾN LƯỢC: HIỂN THỊ GOAL-DRIVEN CỦA AGENT --- */}
      <section className="relative overflow-hidden bg-[#00203F] rounded-[32px] p-7 lg:p-9 text-white shadow-2xl border border-white/10 group transition-all duration-500 hover:border-mekong-cyan/30">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-8">
          {/* KHỐI MỤC TIÊU CỐT LÕI */}
          <div className="flex items-center gap-6 flex-1 min-w-0">
            <div className="relative flex-shrink-0">
              <div className="w-14 h-14 bg-white/5 rounded-2xl flex items-center justify-center text-mekong-cyan border border-white/10 shadow-inner group-hover:border-mekong-cyan/50 transition-all">
                <Target
                  size={28}
                  strokeWidth={2.5}
          ...
          {/* NÚT THIẾT LẬP MỤC TIÊU (DÀNH CHO NGƯỜI GIÁM SÁT) */}
          <div className="flex-shrink-0">
            <button className="flex items-center gap-3 bg-mekong-cyan text-mekong-navy px-8 h-14 rounded-2xl font-black text-[12px] uppercase tracking-widest shadow-xl hover:scale-[1.02] transition-all group/btn">
              <SlidersHorizontal
                size={16}
                strokeWidth={3}
                className="group-hover/btn:rotate-180 transition-transform"
              />
              <span>Điều chỉnh ngưỡng mặn</span>
            </button>
          </div>
        </div>
      </section>

      {/* --- 2. HÀNG CHỈ SỐ THỦY VĂN: TÍCH HỢP TRẠNG THÁI PHÂN TÍCH --- */}
      <div className="grid grid-cols-12 gap-8">
        {/* ĐỘ MẶN TRỰC TIẾP */}
        <Card
          isHoverable
          className="col-span-12 lg:col-span-4 p-8 border-none shadow-soft group relative overflow-hidden bg-white min-h-[280px] flex flex-col justify-between"
        >
          <div className="absolute -top-10 -right-10 w-40 h-40 bg-mekong-mint/10 rounded-full blur-3xl group-hover:bg-mekong-mint/20 transition-all" />
        ...
        <Card
          isHoverable
          className="col-span-12 lg:col-span-4 p-8 border-none shadow-soft group bg-white min-h-[280px] flex flex-col justify-between border-t-4 border-t-mekong-critical/20"
        >
        ...
        {/* GIÓ & HƯỚNG GIÓ CHƯỚNG */}
        <Card
          isHoverable
          className="col-span-12 lg:col-span-4 p-8 border-none shadow-soft group bg-white min-h-[280px] flex flex-col justify-between relative overflow-hidden"
        >
          ...
          <div className="relative z-10 pt-6 border-t border-slate-50 flex items-center gap-2">
            ...
            <span className="text-[11px] font-bold text-slate-500 uppercase tracking-widest leading-none">
              Đang đẩy mặn vào nội đồng
            </span>
          </div>
        </Card>
      </div>

      {/* --- 3. KHU VỰC TRUNG TÂM: BẢN ĐỒ & TRÍ TUỆ NHÂN TẠO --- */}
      <div className="grid grid-cols-12 gap-8 items-start">
        {/* BẢN ĐỒ ĐIỂM NÓNG XÂM NHẬP MẶN (8 CỘT) */}
        <div className="col-span-12 lg:col-span-8">
          <Card
            padding="none"
            className="h-[520px] relative overflow-hidden rounded-[40px] shadow-soft border-none"
          >
        ...
        {/* MEKONG AI SENTINEL: LUỒNG SUY NGHĨ CỦA AGENT (4 CỘT) */}
        <div className="col-span-12 lg:col-span-4 space-y-8">
          <AISentinel />
        </div>
      </div>

      {/* --- 4. NHẬT KÝ HÀNH ĐỘNG TỰ TRỊ & HẠ TẦNG --- */}
      <div className="grid grid-cols-12 gap-8 items-stretch">
        {/* NHẬT KÝ HÀNH ĐỘNG TỰ TRỊ CỦA AI */}
        <Card
          variant="white"
          className="col-span-12 lg:col-span-8 p-8 border-none shadow-soft flex flex-col rounded-[32px]"
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              {
                time: "14:15",
                title: "Đóng cống Hòa Định",
                desc: "Đã kích hoạt PLC đóng cống #04 để ngăn độ mặn 0.8g/L xâm nhập.",
                color: "border-mekong-navy",
              },
              {
                time: "13:50",
                title: "Cảnh báo SMS vùng",
                desc: "Gửi thông báo khẩn tới 1,240 hộ trồng sầu riêng tại Bến Tre.",
                color: "border-mekong-teal",
              },
              {
                time: "12:30",
                title: "Tối ưu hóa Trạm bơm",
                desc: "Điều chỉnh lưu lượng bơm tại Cai Lậy để tối ưu nước ngọt.",
                color: "border-mekong-cyan",
              },
            ].map((action, i) => (
              <div
                key={i}
                className={`flex flex-col justify-between gap-6 p-8 rounded-[32px] bg-slate-50/50 border-l-[8px] ${action.color} group hover:bg-white hover:shadow-2xl transition-all cursor-pointer relative overflow-hidden`}
              >
        ...
        {/* TRẠNG THÁI HẠ TẦNG & ĐỘ CHÍNH XÁC MÔ HÌNH */}
        <Card
          variant="white"
          className="col-span-12 lg:col-span-4 p-8 border-none shadow-soft rounded-[32px] flex flex-col"
        >
          ...
          <div className="mt-10 pt-8 border-t border-slate-50 flex flex-col items-center justify-center gap-4">
            <div className="flex items-center gap-4 px-6 py-2.5 bg-slate-50 rounded-full border border-slate-100 shadow-inner group hover:bg-white transition-all">
