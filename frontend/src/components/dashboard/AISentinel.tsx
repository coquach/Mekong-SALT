...
const ReasoningLog = ({ time, text, isDone = false, isActive = false }: { 
  time: string, 
  text: string, 
  isDone?: boolean, 
  isActive?: boolean 
}) => (
  <div className={`flex gap-4 group transition-all duration-500 ${isActive ? 'scale-105' : 'opacity-80'}`}>
    <span className="text-[10px] font-mono text-slate-500 mt-1 whitespace-nowrap">{time}</span>
    <div className="flex-1">
      <p className={`text-[13px] leading-relaxed italic ${isActive ? 'text-mekong-cyan font-bold' : 'text-slate-300'}`}>
        {text}
      </p>
...
export const AISentinel = () => {
  ...
  // Hiệu ứng đếm ngược đơn giản để UI trông "sống" hơn
  useEffect(() => {
  ...
  return (
    <div className="bg-mekong-navy rounded-[32px] p-8 text-white flex flex-col h-full shadow-2xl relative overflow-hidden border border-white/5">
      {/* 1. Hiệu ứng Glow nền để tạo cảm giác công nghệ cao */}
      ...
      <div className="flex items-center gap-4 mb-10 relative z-10">
        ...
        <div>
          <h3 className="text-lg font-black tracking-tight leading-none">Mekong AI Sentinel</h3>
          <div className="flex items-center gap-2 mt-2">
            ...
            <span className="text-[10px] font-black text-mekong-mint uppercase tracking-[0.2em]">Active Reasoning</span>
          </div>
        </div>
      </div>

      {/* 3. Reasoning Logs Area */}
      <div className="flex-1 space-y-8 overflow-y-auto pr-2 custom-scrollbar relative z-10">
        <ReasoningLog 
          time="14:28" 
          text='"Analyzing Hòa Định sensor upstream data... correlating with Vũng Tàu tidal forecast."' 
          isDone 
        />
        <ReasoningLog 
          time="14:29" 
          text='"Probability of threshold breach at Bến Tre Station 04 rising to 78% in T+45 mins."' 
          isDone 
        />
        ...
        <div className="pt-2 border-t border-white/10">
          <div className="flex items-center gap-2 mb-4 text-mekong-cyan">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest">Generating Mitigation Plan:</span>
          </div>
          <div className="space-y-4 pl-4 border-l-2 border-mekong-cyan/20">
            <div className="flex items-start gap-3">
              ...
              <p className="text-xs font-medium text-slate-300">Initialize gate closure sequence at Node 09.</p>
            </div>
            <div className="flex items-start gap-3">
              <Loader2 size={16} className="text-mekong-cyan mt-0.5 animate-spin" />
              <p className="text-xs font-bold text-mekong-cyan">Dispatch SMS payload to local agricultural co-ops.</p>
            </div>
          </div>
        </div>
      </div>

      {/* 4. Progress & Countdown Section */}
      <div className="mt-10 pt-8 border-t border-white/10 relative z-10">
        ...
        <div className="flex justify-between items-center">
          <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">
            Next Assessment in <span className="text-white">{seconds}s</span>
          </p>
          <div className="flex gap-1">
            {[1, 2, 3].map(i => (
              <div key={i} className={`w-1 h-1 rounded-full ${i <= 2 ? 'bg-mekong-cyan' : 'bg-slate-700'}`} />
            ))}
          </div>
        </div>
      </div>
